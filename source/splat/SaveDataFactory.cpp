...
void SaveDataFactory::updateCRC() { 
    *(uint32_t*)&m_saveData[0x8] = crc32(0L, &m_saveData[HEADER_SIZE], m_bodySize); 
}
...
const uint8_t* SaveDataFactory::getSaveDecodedPtr() { 
    return m_saveData.get(); 
}
...
size_t SaveDataFactory::getEncodedSaveFileSize() { 
    return m_fileSize_encoded; 
}

int SaveDataFactory::getSaveVersion() { 
    return m_version; 
}
